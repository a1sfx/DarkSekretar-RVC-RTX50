...
class VC(object):
    ...
    def vc(
        ...
        if protect < 0.5 and pitch != None and pitchf != None:
            ...
            # Исправлено для mismatched tensors
        min_len = min(feats.shape[1], feats0.shape[1], pitchff.shape[1])
        feats = feats[:, :min_len] * pitchff[:, :min_len] + feats0[:, :min_len] * (1 - pitchff[:, :min_len])
