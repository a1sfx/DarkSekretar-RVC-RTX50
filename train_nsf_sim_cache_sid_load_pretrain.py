...
def run(rank, n_gpus, hps):
    ...
    # Обычный sampler без distributed
...
def run(rank, n_gpus, hps):
    ...
    try:  # 如果能加载自动resume
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path(hps.model_dir, 'D_*.pth') if os.path.exists(hps.model_dir) and len(utils.latest_checkpoint_path.__code__) > 0 else '', net_d, optim_d
        )  # D多半加载没事
        if rank == 0:
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(
                ...
                [train_loader, None],
                logger,
                [None, None],  # writer и writer_eval отключены
                cache,
            )
        else:
            train_and_evaluate(
